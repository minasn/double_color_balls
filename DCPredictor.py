...
class DCPredictor:
	...
	def loadData(self):
		data = funcs.get_data_txt(self.datafile)
		self.balls_list = list(map(lambda x: x[3:], data))
		for days in range(len(self.balls_list)):
			for i in range(7):
				self.balls_list[days][i] = int(self.balls_list[days][i])

	def getOnePosBall(self,n):
		self.n=n
		self.num_l=[]
		for balls in self.balls_list:
			self.num_l.append(balls[n])
	...
	def lstm(self, batchSize=None):
		...
		output_rnn, final_states = tf.nn.dynamic_rnn(cell, inputRnn, initial_state=initState,
			...
			for i in range(100):
				...
				while end < len(self.train_x):
					_, loss_ = sess.run([train_op, loss],
										feed_dict={self.X: self.train_x[start:end], self.Y: self.train_y[start:end]})
